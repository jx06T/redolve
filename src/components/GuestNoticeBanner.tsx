...
export const GuestNoticeBanner: React.FC = () => {
  ...
  return (
    <div className="mb-4 p-3.5 sm:p-4 rounded-2xl bg-primary-50/70 border border-primary-200/60 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs transition-all animate-in fade-in duration-200">
      {/* Left Icon & Description */}
      <div className="flex items-start space-x-3">
        <div className="p-2 rounded-xl bg-primary-100 text-primary shrink-0 mt-0.5 sm:mt-0">
          <Cloud className="w-4 h-4" />
        </div>
        <div className="space-y-0.5">
          <div className="font-bold text-text-main flex items-center space-x-2">
            <span>本機訪客試用模式</span>
            <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-primary-100/70 text-primary">
              未登入
            </span>
          </div>
      ...
      <div className="flex items-center space-x-2 self-end sm:self-center shrink-0">
        ...
        <button
          ...
          className="p-1.5 rounded-xl text-text-muted hover:text-text-main hover:bg-neutral-200/50 transition-colors"
          title="暫時隱藏提示"
          aria-label="暫時隱藏提示"
        >
          <X className="w-4 h-4" />
